...
-- In the below query grouping is done into result sets respectively.
-- 1. global dimensions
...
-- https://www.sqlservertutorial.net/sql-server-window-functions/sql-server-row_number-function/    to better understand row_number function
...
select customerid, (sum(totalpurchaseytd) over (partition by customerid) / sum(totalpurchaseytd) over ()) percentage 
from customer_demo cd 

-- Find the customerset based on percentage contribution on the total sales (YTD)

select maritalstatus, yearlyincome, education, sum(v.percentage) as percentage_of_purchase 
from customer_demo cd
join 
